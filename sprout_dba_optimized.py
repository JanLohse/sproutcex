...
def escape_prefix_optim(graph_dict, word, initial_state, infinity_run_cache):
    success, result, _ = infinity_run_optim(graph_dict, word, initial_state, infinity_run_cache)
    if success:
        return None
    else:
...
def escapes_optim(graph_dict, plus, minus, initial_state, infinity_run_cache, affected_words, escaping_edge_to_words,
                  escaping, escaping_set):
...
def extend_optim(graph_dict, plus, initial_state, infinity_run_cache):
    escape_strings = {}
    for word in plus:
        success, count, state = infinity_run_optim(graph_dict, word, initial_state, infinity_run_cache)
        if not success:
...
def aut_optim(graph_dict, minus, initial_state, infinity_run_cache):
    accepting_states = büchi_marking_optim(graph_dict, minus, initial_state, infinity_run_cache)
...
def sprout_dba_optim(plus, minus, square_threshold=False):
    ...
    while escaping:
        ...
        if len(u) > threshold:
            return aut_optim(extend_optim(graph_dict, plus, initial_state, infinity_run_cache), minus, initial_state,
                             infinity_run_cache)
        ...
        for q in sorted(graph_dict):
            ...
            consistent, cache_update = büchi_consistent_optim(graph_dict, plus, minus, initial_state,
                                                              infinity_run_cache)
            if consistent:
